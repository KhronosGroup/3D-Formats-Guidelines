...
The model is available in multiple variants:
1. `StainedGlassLamp-JPG-PNG-Simple.glb` uses no extensions for the widest compatibility. Base color textures with alpha were kept in PNG format, while the rest were converted into JPG.
1. `StainedGlassLamp-JPG-PNG-Transmission.glb` same as above, with the addition of [KHR_materials_clearcoat](https://github.com/KhronosGroup/glTF/blob/master/extensions/2.0/Khronos/KHR_materials_clearcoat/README.md), [KHR_materials_transmission](https://github.com/KhronosGroup/glTF/blob/master/extensions/2.0/Khronos/KHR_materials_transmission/README.md), and [KHR_materials_variants](https://github.com/KhronosGroup/glTF/blob/master/extensions/2.0/Khronos/KHR_materials_variants/README.md).
1. `StainedGlassLamp-JPG-PNG-Volume.glb` same as above, with the addition of [KHR_materials_ior](https://github.com/KhronosGroup/glTF/pull/1718) and [KHR_materials_volume](https://github.com/KhronosGroup/glTF/pull/1726).
1. `StainedGlassLamp-KTX-BasisU-Simple.glb` uses the same settings as `StainedGlassLamp-JPG-PNG-Simple.glb` except textures are compressed into KTX format using high-quality settings and the extension [KHR_texture_basisu](https://github.com/KhronosGroup/glTF/blob/master/extensions/2.0/Khronos/KHR_texture_basisu/).
1. `StainedGlassLamp-KTX-BasisU-Transmission.glb` uses the same settings as `StainedGlassLamp-JPG-PNG-Transmission.glb` except textures are in KTX format.
1. `StainedGlassLamp-KTX-BasisU-Volume.glb` uses the same settings as `StainedGlassLamp-JPG-PNG-Volume.glb` except textures are in KTX format.
1. `StainedGlassLamp-KTX-BasisU-Volume-ETC1S.glb` uses the same settings as `StainedGlassLamp-KTX-BasisU-Volume.glb` except KTX textures were compressed using only ETC1S.
1. `StainedGlassLamp-PNG-2048.glb` uses the same settings as `StainedGlassLamp-JPG-PNG-Volume.glb` except textures are all in PNG format, and are the highest resolution (mostly 2048).
1. `StainedGlassLamp-PNG-Pathtracer.glb` uses the same settings as `StainedGlassLamp-PNG-2048.glb` except all emissive textures have been disabled, and the emissiveFactor for the light bulbs has been set to (100,100,100).
...
All the models except the -Simple versions use the extension [KHR_materials_variants](https://github.com/KhronosGroup/glTF/blob/master/extensions/2.0/Khronos/KHR_materials_variants/README.md) to add a variant with the emissive textures disabled to simulate the lights being turned off. 
...
The "KTX" models use [Basis Universal](https://github.com/KhronosGroup/KTX-Software) texture compression and the extension [KHR_texture_basisu](https://github.com/KhronosGroup/glTF/blob/master/extensions/2.0/Khronos/KHR_texture_basisu/). 

For most of the KTX models, the aim was to achieve the same visual quality as the original JPG/PNG textures, while compressing both file size and GPU memory size. An exception is `StainedGlassLamp-KTX-BasisU-Volume-ETC1S.glb` which only uses ETC1S for the smallest possible file size, while sacrificing some visual quality.

The other models' textures were compressed from PNG source into KTX2 using [toktx](https://github.com/KhronosGroup/KTX-Software) via a combination of compression settings that favored high quality. UASTC was used for ORM and Normal textures and ETCS1 for Base Color and Emissive textures, however exceptions were made based on analyzing results... the `glass_basecolor-alpha` and `glass_emissive` textures were changed from ETC1S to UASTC since these were prominently displayed on the model and contained a lot of color variation. 
...
The model `StainedGlassLamp-PNG-Pathtracer.glb` is setup for use in non-rasterized renderers, which often require a few adjustments. E.g. emissive textures that simulate light bounce have been removed, allowing the path tracing to create more accurate bounce lighting. The emissiveFactor for the light bulbs has been increased to 100 to cast enough internal lighting into the model.
